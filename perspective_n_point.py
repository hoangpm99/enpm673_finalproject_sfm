...
def perspective_n_points(X, x):
    ...
    for i in range(n):
        X_i = X[i]
        x_i = x[i]
        A[2*i] = [X_i[0], X_i[1], X_i[2], 1, 0, 0, 0, 0, -x_i[0]
                  * X_i[0], -x_i[0]*X_i[1], -x_i[0]*X_i[2], -x_i[0]]
        A[2*i+1] = [0, 0, 0, 0, X_i[0], X_i[1], X_i[2], 1, -x_i[1]
                    * X_i[0], -x_i[1]*X_i[1], -x_i[1]*X_i[2], -x_i[1]]
...
# Perspective-n-Points algorithm with RANSAC
def perspective_n_points_ransac(X, x, ransac_iter, ransac_threshold):
    ...
    for _ in range(ransac_iter):
        ...
        # Need 6 points to perform linear matching
        indexes = np.random.choice(len(masked_X), size=6, replace=False)
        R_estimate, C_estimate = perspective_n_points(
            masked_X[indexes], masked_x[indexes])

        # Project 3D points to 2D using the estimated pose and calculate error
        proj = (masked_X - C_estimate) @ R_estimate.T
        errors = np.sqrt((proj[:, 0] / proj[:, 2] - masked_x[:, 0])
                         ** 2 + (proj[:, 1] / proj[:, 2] - masked_x[:, 1])**2)

        inliers = np.zeros(X.shape[0])
        inliers[mask_idx] = errors < ransac_threshold
...
# Perspective-n-Points algorithm with nonlinear optimization


def perspective_n_points_nonlinear(R, C, X, x):
    n = X.shape[0]
    ...
    for i in range(n_iters):
        R_i = quaternion_to_rotation(p[3:])
        C_i = p[:3]
        ...
        J = np.zeros(7)
        for j in range(n):
            dfdp = compute_pose_jacobian(p, X[j, :])
            H = H + dfdp.T @ dfdp
    ...
    R_refined = quaternion_to_rotation(p[3:])
    C_refined = p[:3]
